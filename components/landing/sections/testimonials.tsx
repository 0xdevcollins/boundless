...
export default function Testimonials() {
  ...
  const nextSlide = () => {
    setActiveIndex((prevIndex) => (prevIndex + itemsPerView >= testimonials.length ? 0 : prevIndex + 1));
  };

  const prevSlide = () => {
    setActiveIndex((prevIndex) => (prevIndex === 0 ? testimonials.length - itemsPerView : prevIndex - 1));
  };
  ...
  if (visibleTestimonials.length < itemsPerView) {
    const remainingItems = testimonials.slice(0, itemsPerView - visibleTestimonials.length);
        ...
        <div className="container max-w-6xl mx-auto px-4 md:px-6">
          ...
          >
            <div className="relative inline-block">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tighter mb-6">What People Say About Boundless</h2>
              <AnimatedUnderline isInView={isInView} />
            </div>
            <p className="text-lg text-muted-foreground mt-6">Join our growing community of creators and backers</p>
          </motion.div>
              ...
              {testimonials.map((testimonial, index) => (
                <Button
                  ...
                  className={`w-2 h-2 rounded-full mx-1 p-0 min-w-0 ${
                    index >= activeIndex && index < activeIndex + itemsPerView ? 'bg-primary' : 'bg-muted'
                  }`}
