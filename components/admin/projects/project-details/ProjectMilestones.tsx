...
export function ProjectMilestones({ milestones }: ProjectMilestonesProps) {
	...
	const [selectedMilestone, setSelectedMilestone] =
		useState<MilestoneProp | null>(null);
			...
			toast.error("Error", {
				description:
					error instanceof Error
						? error.message
						: "Failed to update milestone status. Please try again.",
			});
		} finally {
			setIsLoading(null);
		}
								...
								<div className="space-y-4">
									...
									<div className="flex items-center justify-between text-sm">
										...
										{milestone.attachments.length > 0 && (
											<span className="text-muted-foreground">
												{milestone.attachments.length} attachment
												{milestone.attachments.length !== 1 ? "s" : ""}
											</span>
										)}
									</div>
								</div>
				...
				<Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
					<DialogContent className="max-w-2xl">
						<DialogHeader>
							...
							<DialogDescription>
								Milestone details and attachments
							</DialogDescription>
						</DialogHeader>
						...
						<div className="space-y-4">
							...
							{selectedMilestone.dueDate && (
								<div>
									...
									<p className="text-sm">
										{format(new Date(selectedMilestone.dueDate), "PPP")}
									</p>
								</div>
							)}
							...
							<div className="flex gap-2">
								...
								{selectedMilestone.status !== "IN_PROGRESS" &&
									selectedMilestone.status !== "COMPLETED" && (
										<Button
											onClick={() =>
												handleUpdateStatus(selectedMilestone.id, "IN_PROGRESS")
											}
											disabled={isLoading === selectedMilestone.id}
											className="gap-1"
										>
