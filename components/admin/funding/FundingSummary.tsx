...
import { DollarSign, Clock, CheckCircle2, XCircle, RefreshCw } from "lucide-react"
...
export function FundingSummary({ data }: FundingSummaryProps) {
  ...
  const totalAmount = totalCompleted + totalPending + totalFailed + totalRefunded
  const totalCount = countCompleted + countPending + countFailed + countRefunded
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <Card className="lg:col-span-1">
        <CardContent className="pt-6">
          ...
          <p className="text-xs text-muted-foreground mt-2">{countCompleted} transactions</p>
        </CardContent>
      </Card>
      ...
      <Card className="lg:col-span-1">
        <CardContent className="pt-6">
          ...
          <p className="text-xs text-muted-foreground mt-2">{countPending} transactions</p>
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold">${totalFailed.toLocaleString()}</h3>
            </div>
            <div className="h-12 w-12 rounded-full bg-red-100 flex items-center justify-center">
              <XCircle className="h-6 w-6 text-red-600" />
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-2">{countFailed} transactions</p>
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h3 className="text-2xl font-bold">${totalRefunded.toLocaleString()}</h3>
            </div>
            <div className="h-12 w-12 rounded-full bg-blue-100 flex items-center justify-center">
              <RefreshCw className="h-6 w-6 text-blue-600" />
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-2">{countRefunded} transactions</p>
        </CardContent>
      </Card>
    </div>
  )
