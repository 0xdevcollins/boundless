...
export function MilestonesTable({ milestones, currentPage, totalPages }: MilestonesTableProps) {
  ...
  const handleUpdateStatus = async (milestoneId: string, status: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/milestones/${milestoneId}/status`, {
        method: "PATCH",
        headers: {
                  ...
                  <TableRow key={milestone.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Avatar className="h-6 w-6">
                          <AvatarImage
                            src={milestone.project.user?.image || "/placeholder.svg"}
                            alt={milestone.project.user?.name}
                          />
                          <AvatarFallback>{milestone.project.user?.name?.charAt(0) || "U"}</AvatarFallback>
                        </Avatar>
                        <Link href={`/admin/projects/${milestone.project.id}`} className="text-sm hover:underline">
                    ...
                    <TableCell>
                      <div className="w-full max-w-[100px]">
                        <div className="text-xs mb-1">{milestone.progress}%</div>
                        <Progress value={milestone.progress} className={`h-2 ${getStatusColor(milestone.status)}`} />
                      </div>
                    </TableCell>
                    <TableCell>
                      {milestone.dueDate ? (
                        <span className="text-sm">{format(new Date(milestone.dueDate), "MMM d, yyyy")}</span>
                      ) : (
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Milestone details and progress</DialogDescription>
          </DialogHeader>
          ...
          {selectedMilestone && (
            <div className="space-y-4">
              ...
              <div className="text-sm">
                ...
                <Link href={`/admin/projects/${selectedMilestone.project.id}`} className="text-primary hover:underline">
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Files attached to milestone: {selectedMilestone?.title}</DialogDescription>
          </DialogHeader>

          {selectedMilestone && (
            <div className="space-y-4">
              {selectedMilestone.attachments.length === 0 ? (
                <div className="text-center py-6 text-muted-foreground">No attachments found for this milestone</div>
              ) : (
                <div className="space-y-2">
              ...
              <div className="pt-2">
                <Button variant="outline" size="sm" onClick={() => setShowAttachments(false)}>
                  Back to Details
                </Button>
              </div>
            </div>
