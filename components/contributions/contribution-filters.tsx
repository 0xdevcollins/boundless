...
export function ContributionFilters({
  ...
  const selectedCategory = categoryFilter === 'all' ? 'Category (All)' : categoryFilter;
      ...
      <div className="flex flex-col md:flex-row justify-between gap-4 mb-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <Filter
            ...
            onChange={(value: string) => setCategoryFilter(value === 'Category (All)' ? 'all' : value)}
            label="Category"
            allLabel="Category (All)"
          />

          <SortByFilter sortOption={sortOption} setSortOption={setSortOption} options={sortOptions} />
        </div>
      </div>
    </div>
  );
