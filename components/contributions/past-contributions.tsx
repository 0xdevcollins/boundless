...
export function PastContributions({ projects, navigateToProject, itemsPerPage = 6 }: PastContributionsProps) {
          ...
          <TableBody>
            {paginatedProjects.map((project) => (
              <TableRow key={project.id}>
                ...
                <TableCell>
                  {project.funded ? (
                    <Badge className="bg-blue-100 text-blue-800 border-blue-200">Funded</Badge>
                  ) : (
                    <Badge variant="outline">Not Funded</Badge>
                  )}
                </TableCell>
                <TableCell>
                  <Button variant="ghost" size="sm" onClick={() => navigateToProject(project.id)}>
                    View
                  </Button>
                </TableCell>
              </TableRow>
