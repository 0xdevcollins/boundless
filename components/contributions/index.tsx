...
import { sortActiveProjects, sortComments, sortPastProjects } from '@/lib/utils';
import type {
  ActiveProject,
  ContributionStats,
  PastProject,
...
export default function MyContributionsPage({ username }: { username?: string }) {
  ...
  const sortedActiveProjects = activeProjects ? sortActiveProjects(activeProjects, sortOption) : [];
  const sortedPastProjects = pastProjects ? sortPastProjects(pastProjects, sortOption) : [];
  ...
  const isOnContributionsPage = pathname === '/contributions';

  return (
    <div className="container mx-auto py-8 max-w-7xl">
      <h1 className="text-3xl font-bold mb-6">
        {isOnContributionsPage ? 'My Contributions' : username ? `Hi, ${username.split(' ')[0]}!` : 'Dashboard'}
      </h1>

      {/* Summary Section */}
      {isLoadingStats ? <LoadingState type="stats" /> : stats ? <ContributionStatsComponent stats={stats} /> : null}
      ...
      {(activeTab === 'all' || activeTab === 'votes') && (
        <>
          ...
          {isLoadingActive ? (
            ...
            <ActiveContributions projects={sortedActiveProjects} navigateToProject={navigateToProject} />
          )}
        </>
      )}
      ...
      {(activeTab === 'all' || activeTab === 'votes') && (
        <>
          ...
          {isLoadingPast ? (
            ...
            <PastContributions projects={sortedPastProjects} navigateToProject={navigateToProject} />
          )}
        </>
      )}
