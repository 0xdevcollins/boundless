...
export const WalletProtectedAction = () => {
  ...
  const { protectedAction, showModal, setShowModal } = useWalletProtection(handleProtectedAction);

  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium">Wallet Protected Action</h3>
      <p className="text-sm text-muted-foreground">This button will show a modal to connect your wallet if needed.</p>
      <Button onClick={protectedAction}>Execute Protected Action</Button>

      {/* Render the wallet connection modal */}
      <WalletConnectionModal isOpen={showModal} onClose={() => setShowModal(false)} onSuccess={protectedAction} />
    </div>
  );
};
