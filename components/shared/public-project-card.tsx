...
import { calculateDaysLeft, calculateFundingPercentage, cn, formatValidationStatus } from '@/lib/utils';
...
export function PublicProjectCard({ project, className }: ProjectCardProps) {
  ...
  const projectLink = isLoading ? '#' : isOwner ? `/projects/edit/${project.id}` : `/projects/${project.id}`;
  ...
  return (
    ...
    >
        ...
        <div className="flex justify-between items-start">
          ...
          {isFunded && daysLeft > 0 && (
            <div className="flex items-center text-sm text-muted-foreground">
              ...
              <span className={daysLeft <= 5 ? 'text-amber-500' : ''}>{daysLeft} days left</span>
            </div>
          )}
        </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex justify-between mt-2 text-sm">
              <div className="flex items-center">
                {fundingPercentage >= 80 && <Sparkles className="h-3.5 w-3.5 text-amber-500 mr-1" />}
                <span className={fundingPercentage >= 80 ? 'font-medium' : ''}>{fundingPercentage}% funded</span>
              </div>
            </div>
          </div>
        )}
      ...
      <CardFooter>
        ...
        >
          <Link href={projectLink} className="w-full flex items-center justify-center">
