...
export function VoteButton({ projectId, initialVoteCount, initialUserVoted, compact = false }: VoteButtonProps) {
  const [voteCount, setVoteCount] = useState<number | undefined>(initialVoteCount);
  const [userVoted, setUserVoted] = useState<boolean | undefined>(initialUserVoted);
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(result.voted ? 'Vote registered!' : 'Vote removed!', {
          description: result.voted ? 'Thank you for supporting this project.' : 'Your vote has been removed.',
        });
  ...
  return (
    <div className={`flex items-center ${compact ? 'gap-1' : 'gap-2'}`}>
      ...
      >
        <ThumbsUp className={`${compact ? 'h-3.5 w-3.5' : 'h-4 w-4'} ${userVoted ? 'fill-current' : ''}`} />
