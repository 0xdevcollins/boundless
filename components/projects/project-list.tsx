  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {projects.map((project) => {
        const userVoted = session?.user?.id ? project.votes.some((vote) => vote.userId === session.user?.id) : false;

        return <ProjectCard key={project.id} project={project} userVoted={userVoted} />;
      })}
    </div>
  );
}
