...
export function FundProjectDialog({ projectId, isOpen, onClose, onSuccess }: FundProjectDialogProps) {
  ...
  const handleFund = async () => {
    ...
    try {
      ...
      const tx = await contractClient.fund_project({
        ...
        funder: publicKey,
        token_contract: 'CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC', // Native XLM asset address
      });
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Enter the amount you want to fund in USD</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            />
            {xlmPrice && amount && (
              <p className="text-sm text-gray-500">≈ {(Number(amount) / xlmPrice).toFixed(2)} XLM</p>
            )}
          </div>
          <Button onClick={handleFund} disabled={isLoading || !amount || Number(amount) <= 0}>
