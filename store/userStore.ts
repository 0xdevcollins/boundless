...
export interface UserState {
  ...
  isProjectTeamMember: (projectId: string, projectTeamMembers: TeamMemberBasic[]) => boolean;
  hasCreatedProject: () => boolean;
...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  fetchUserProfile: async () => {
    try {
      ...
      const hasCreatedProject = Array.isArray(userData.projects) && userData.projects.length > 0;
  ...
  isProjectTeamMember: (projectId: string, projectTeamMembers: TeamMemberBasic[]) => {
    const { user } = get();
  ...
  hasCreatedProject: () => {
    ...
    return !!user.hasCreatedProject || (Array.isArray(user.projects) && user.projects.length > 0);
  },
  ...
  getTotalContributions: () => {
    ...
    if (previousContributions > 0) {
      growthPercentage = ((currentContributions - previousContributions) / previousContributions) * 100;
    }
  ...
  getActiveProjects: () => {
    ...
    const activeProjects = user.projects.filter(
      (project) => project.ideaValidation === 'VALIDATED' && !project.completedAt,
    );
    ...
    for (const project of activeProjects) {
      if (project.milestones) {
        pendingMilestones += project.milestones.filter((milestone) => milestone.status === 'PENDING').length;
      }
    }
  ...
  getSuccessfulExits: () => {
    ...
    if (user?.successfulExits !== undefined && user?.totalExitValue !== undefined) {
      return {
    ...
    const completedProjects = user.projects.filter((project) => !!project.completedAt);
