...
export async function POST(request: Request) {
  try {
    ...
    if (!session?.user?.id) {
      return NextResponse.json({ error: 'You must be signed in to react to comments' }, { status: 401 });
    }
    ...
    let reaction: { id: string; type: ReactionType; userId: string } | undefined;
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!session?.user?.id) {
      return NextResponse.json({ error: 'You must be signed in to remove reactions' }, { status: 401 });
    }
    ...
    if (!existingReaction) {
      return NextResponse.json({ error: 'No reaction found to remove' }, { status: 404 });
    }
