...
import { ChevronDown, ChevronUp, Edit, Reply, ThumbsDown, ThumbsUp, Trash2 } from 'lucide-react';
...
export function Comment({ comment, allComments, onCommentUpdated, currentUserId, level }: CommentProps) {
  ...
  const handleReact = async (type: 'LIKE' | 'DISLIKE') => {
    ...
    try {
      ...
      const isRemovingReaction = (type === 'LIKE' && hasLiked) || (type === 'DISLIKE' && hasDisliked);
    ...
    <Card className={`border-l-4 ${level > 0 ? 'border-l-muted' : 'border-l-primary'}`}>
      <CardContent className="p-4">
        <div className="flex gap-3">
          ...
          <Avatar className="h-8 w-8">
            ...
            <AvatarFallback>{comment.user.name?.substring(0, 2).toUpperCase() || 'U'}</AvatarFallback>
          </Avatar>
      ...
      <CardFooter className="p-2 px-4 flex flex-wrap gap-2 justify-between">
        ...
        <div className="flex gap-1">
          ...
          {isOwnComment && (
            <>
              <Button variant="ghost" size="sm" className="px-2" onClick={() => setIsEditing(!isEditing)}>
      ...
      {replies.length > 0 && (
        <div className="px-4 pb-4">
          <Button variant="ghost" size="sm" className="mb-2 text-xs" onClick={() => setShowReplies(!showReplies)}>
            {showReplies ? (
              <>
