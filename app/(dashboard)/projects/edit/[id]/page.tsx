...
export default function ProjectPage() {
  ...
  const isTeamMember =
    project?.userId === session?.user?.id || project?.teamMembers.some((member) => member.userId === session?.user?.id);
  ...
  if (error) {
    return <div className="container py-8 text-center text-destructive">Error: {error}</div>;
  }
      ...
      <div className="container relative z-10 -mt-32 px-4 sm:px-6 lg:px-8">
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-2 text-sm text-muted-foreground">
                    <Badge variant="secondary" className="flex items-center gap-1">
                      <Users className="h-3 w-3" /> {project._count.votes} Supporters
                    </Badge>
                    <Badge variant="secondary" className="flex items-center gap-1">
                      <Wallet className="h-3 w-3" /> ${project.fundingGoal.toLocaleString()} Goal
                    </Badge>
                  </div>
                </div>
        ...
        <Tabs defaultValue="milestones" className="mt-6">
          ...
          <TabsContent value="team">
            <TeamSection teamMembers={project.teamMembers} isTeamMember={!!isTeamMember} projectId={project.id} />
          </TabsContent>

          <TabsContent value="voting">
            <VotingSection
              projectId={project.id}
              initialUserVoted={project.votes.some((vote) => vote.userId === session?.user?.id)}
          ...
          <TabsContent value="milestones">
            <MilestoneTracker isTeamMember={!!isTeamMember} projectId={project.id} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
