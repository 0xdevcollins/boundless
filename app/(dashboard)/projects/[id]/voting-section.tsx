...
export function VotingSection({
	...
	const progressPercentage = Math.min(
		Math.round((initialVoteCount / votingGoal) * 100),
		100,
	);
					...
					<div className="space-y-6">
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								...
								>
									{ideaValidation.charAt(0) +
										ideaValidation.slice(1).toLowerCase()}
								</Badge>
						...
						{ideaValidation === "PENDING" && (
							<div className="space-y-2">
								...
								<p className="text-xs text-muted-foreground">
									This project needs {votingGoal - initialVoteCount} more votes
									to reach the validation threshold
								</p>
							</div>
						)}
						...
						<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
							...
							{votingEnabled ? (
								...
								<div className="text-sm text-muted-foreground">
									Voting is{" "}
									{ideaValidation === "VALIDATED" ? "complete" : "closed"} for
									this project
								</div>
							)}
						</div>
							...
							<div className="rounded-lg bg-muted p-4 flex gap-3">
								...
								<div>
									<h4 className="font-medium">
										Voting is{" "}
										{ideaValidation === "VALIDATED" ? "complete" : "closed"}
									</h4>
				...
				<Card>
					...
					<CardContent>
						<div className="space-y-4">
							...
							{Array.from({ length: Math.min(3, initialVoteCount) }).map(
								(_, i) => {
